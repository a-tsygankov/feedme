...
struct Cat {
    ...
    // 32-char lowercase hex (16 random bytes) + NUL. Phase D — gives
    // each cat a stable identity decoupled from `id`/`slot_id`. The
    // backend matches incoming sync rows by uuid first; (hid, slot_id)
    // is the legacy fallback for pre-Phase-D firmware that never
    // generated one.
    static constexpr int     UUID_LEN            = 32;
    static constexpr int     UUID_CAP            = UUID_LEN + 1;   // +NUL
    static constexpr int64_t DEFAULT_THRESHOLD_S = 5 * 3600;  // 5h

    uint8_t      id   = 0;
    char         name[NAME_CAP] = {0};
    char         slug[SLUG_CAP] = {0};
    char         uuid[UUID_CAP] = {0};   // empty until generated by add() or sync
...
public:
    ...
    void appendLoaded(uint8_t id, const char* name, const char* slug,
        ...
        // Phase D — uuid is empty by default (legacy NVS rows have
        // none yet); SyncService populates it from the first server
        // response, after which subsequent NVS loads pass it back in.
        if (uuid) { strncpy(c.uuid, uuid, Cat::UUID_LEN); c.uuid[Cat::UUID_LEN] = '\0'; }
        else      { c.uuid[0] = '\0'; }
    ...
    // Forced-dirty after a sync replaces the roster from the server
    // response — the appendLoaded path doesn't itself set dirty (it
    // mirrors the boot load pattern), but freshly-arrived sync
    // results need to land in NVS so the next reboot doesn't lose
    // the server-assigned uuids.
