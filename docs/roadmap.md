...
- **Entity model.** One household; 1..N devices (today 1; pass `deviceId` everywhere); 1..N users; 1..N cats. Every new feature must declare scope (household / device / per-cat / per-user) before any code is written, and must follow the **adaptive UI rule**: never offer a selector for an entity with cardinality 1. Scope-per-state table + back-compat backend plan live in [handoff.md § "Entities: household, devices, users, cats"](../handoff.md). Today's firmware treats cat as singular (no cat selector yet, correct since N=1), user as a single hardcoded `"you"` in `PouringView::onComplete` (correct for N=1, will need a selector or per-device signed-in user when N≥2), and `deviceId` is not yet emitted on API calls (placeholder until 2.1 lands `WifiNetwork`). The on-device path from N=1 to N≥2 lands in [feedmeknob-plan.md § Phase D](feedmeknob-plan.md) items **D.5 Cats editor** and **D.6 Users editor**; the cleaner phone-keyboard path lands with the captive portal (Phase 2.4).
